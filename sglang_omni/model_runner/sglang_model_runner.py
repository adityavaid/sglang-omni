    ...
    def _profile_available_bytes(self, pre_model_load_memory: float) -> int:
        ...
        if self.kv_cache_bytes is not None:
            ...
            if self.mambaish_config is not None:
                raise ValueError(
                    "runtime.memory.kv_cache_bytes does not support "
            ...
            if free_bytes < self.kv_cache_bytes:
                raise ValueError(
                    ...
                    f"gpu_id={self.gpu_id}. Lower runtime.memory.kv_cache_bytes "
                    "or free memory on this GPU."
                )
            logger.info(
                "SGLang AR memory profile: gpu_mem_accounting=kv_cache_bytes "
